...
fun Table.firstOrNull(where: SqlExpressionBuilder.() -> Op<Boolean>): ResultRow? = selectAll().where(where).limit(1).firstOrNull()
...
fun <T : Comparable<T>> IdTable<T>.getId(where: SqlExpressionBuilder.() -> Op<Boolean>): T? = firstOrNull(where)?.get(id)?.value
