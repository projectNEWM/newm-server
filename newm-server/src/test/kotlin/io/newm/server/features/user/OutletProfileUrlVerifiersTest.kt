...
class OutletProfileUrlVerifiersTest : BaseApplicationTests() {
    ...
    fun testSpotifyProfileUrlVerifierSuccess() =
        runBlocking {
    ...
    @Test
    @Disabled("This test is disabled because it requires a valid Spotify API token")
    fun testSpotifyProfileUrlVerifierFailure() =
        runBlocking {
            val verifier = SpotifyProfileUrlVerifier(spotifyHttpClient)
            val exception =
                assertThrows<OutletProfileUrlVerificationException> {
                    verifier.verify("https://open.spotify.com/artist/3WrFJ7ztbogyGnTHbHJFl2", "Teh Beatles")
                }
            assertThat(
                exception.message
            ).isEqualTo("Spotify profile name (The Beatles) does not match stageOrFullName (Teh Beatles) for 3WrFJ7ztbogyGnTHbHJFl2")
        }

    @Test
    @Disabled("This test is disabled because it requires a valid Apple Music JWT token")
    fun testAppleMusicProfileUrlVerifierSuccess() =
        runBlocking {
    ...
    @Test
    @Disabled("This test is disabled because it requires a valid Apple Music JWT token")
    fun testAppleMusicProfileUrlVerifierFailure() =
        runBlocking {
            val verifier = AppleMusicProfileUrlVerifier(appleMusicHttpClient)
            val exception =
                assertThrows<OutletProfileUrlVerificationException> {
            ...
            assertThat(
                exception.message
            ).isEqualTo("Apple Music profile name (The Beatles) does not match stageOrFullName (Teh Beatles) for 136975")
        }

    @Test
    @Disabled("This test is disabled because it requires a valid SoundCloud API token")
    fun testSoundCloudProfileUrlVerifierSuccess() =
        runBlocking {
    ...
    @Test
    @Disabled("This test is disabled because it requires a valid SoundCloud API token")
    fun testSoundCloudProfileUrlVerifierFailure() =
        runBlocking {
            val verifier = SoundCloudProfileUrlVerifier(soundCloudHttpClient)
            val exception =
                assertThrows<OutletProfileUrlVerificationException> {
    ...
    fun testCreateAppleMusicKitJWT() =
        runBlocking {
