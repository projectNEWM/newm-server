            ...
            val address =
                if (isMainnet) {
                    Bech32.encode(
                        ...
                        ByteArray(
                            1
                        ) { PAYMENT_STAKE_ADDRESS_SCRIPT_SCRIPT_PREFIX_MAINNET } + paymentAddressCredential.hash +
                            stakeAddressCredential.hash
                    )
                } else {
                    Bech32.encode(
                        "addr_test",
                        ByteArray(
                            1
                        ) { PAYMENT_STAKE_ADDRESS_SCRIPT_SCRIPT_PREFIX_TESTNET } + paymentAddressCredential.hash +
                            stakeAddressCredential.hash
                    )
                }
            return Address(address)
